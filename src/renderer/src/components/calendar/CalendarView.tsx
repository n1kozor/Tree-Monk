  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    kinds[k] ? M.on : 'bg-muted text-muted-foreground hover:text-foreground'
                  )}
                >
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 sm:px-6">
        <button onClick={() => goMonth(-1)} className="flex h-8 w-8 items-center justify-center rounded-md border border-border hover:bg-accent" aria-label="prev">
          <ChevronLeft className="h-4 w-4" />
        </button>
        <h3 className="min-w-[7.5rem] text-center text-lg font-semibold capitalize">{monthName}</h3>
        <button onClick={() => goMonth(1)} className="flex h-8 w-8 items-center justify-center rounded-md border border-border hover:bg-accent" aria-label="next">
          <ChevronRight className="h-4 w-4" />
        </button>
        <button onClick={goToday} className="rounded-md border border-border px-2.5 py-1.5 text-xs font-medium hover:bg-accent">
          {t('calendar.today')}
        </button>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-auto px-4 pb-6 sm:px-6 lg:flex-row">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="grid grid-cols-7 gap-1.5">
            {cells.map((day, i) => {
              if (day == null) return <div key={i} className="min-h-[72px] rounded-lg" />
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSel ? 'border-primary ring-1 ring-primary' : 'border-border hover:border-primary/40 hover:bg-accent/40',
                    evs.length ? 'bg-card' : 'bg-background/50'
                  )}
                >
                  <span
                    className={cn(
